...
```{r, eval=FALSE}
devtools::install_github("wwang-chcn/Giotto@suite_dev")
# Note: do not update GiottoClass during this installation
```

## ONTraC Installation

Since ONTraC is a GNN algorithm that relies on [pytorch-geometric](https://pytorch-geometric.readthedocs.io/en/latest/), we create a new environment here in order not to interfere with other software using `installONTraCEnvironment()`.

```{r, eval=FALSE}
library(Giotto)
installONTraCEnvironment(envname = "giotto_ontrac_env")
```
...
data_path <- file.path("data","ontrac")
dir.create(data_path, recursive=T)

results_folder <- file.path("results","ontrac")
...
We recommend to download these data in advance or download the processing giotto obj from [Zenodo](https://zenodo.org/records/13255549).
...
Option 2 is load the pre-constructed giotto object from [Zenodo](https://zenodo.org/records/13255549).
...
GiottoUtils::package_check("scanpy", repository= "pip:scanpy")
...
If you facing network issue when downloading the raw dataset. Please download the processing giotto obj from [Zenodo](https://zenodo.org/records/13255549), unzip and move it to `results` folder.
...
We have three options for running ONTraC.
Option 1 is running ONTraC through reticulate.
Option 2 is running ONTraC through a shell which is not usable in Windows.
Option 3 is a example for users running ONTraC in a seperate machine, which is usually a more powerful computer with GPU.

### Option 1: run ONTraC through reticulate
...
If you encountered R session aborted, please try option 2.

### Option 2: run ONTraC through shell
...
### Option 3: run ONTraC in a separate machine through shell
...
ONTraC -d results/ontrac/ONTraC_dataset_input.csv --preprocessing-dir results/ontrac/preprocessing_dir --GNN-dir results/ontrac/GNN_dir --NTScore-dir results/ontrac/NTScore_dir --device cuda     --epochs 1000 -s 42 --patience 100 --min-delta 0.001 --min-epochs 50 --lr 0.03     --hidden-feats 4 -k 6 --modularity-loss-weight 0.3 --purity-loss-weight 300 --regularization-loss-weight 0.3 --beta 0.03 2>&1 | tee results/ontrac/merfish_subset.log
```

- Copy the output directories (`results/ontrac/preprocessing_dir`, `results/ontrac/GNN_dir`, and `results/ontrac/NTScore_dir`) back to the machine running Giotto.
...
```{r, echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("images/ontrac/5-1-rawSpatialNTScore.png")
```
...
```{r, echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("images/ontrac/7-heatmapCellTypeCompositionInNicheCluster.png")
```
